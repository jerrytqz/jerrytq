...
const Courses = () => {
    ...
    useEffect(() => {
        ...
        setFetchError(false);
        fetch(`${BACKEND_BASE_DIR}/fetch-courses/?termName=${termName}`, {method: 'GET'})
            .then(response => {
    ...
    return (
        <section className={classes.Container}>
            ...
            <label className={[classes.TermNameSelectWrapper, classes.Min768None].join(' ')}>
                Term
                <select className={classes.TermNameSelect} value={termName} onChange={(event) => { setTermName(event.target.value); }}>
                    {TERM_NAMES.map(termName => <option key={`${termName}0`} value={termName}>{termName}</option>)}
                </select>
            </label>
            <table className={classes.Table}>
                <tbody>
                    <tr>
                        <th className={[classes.TableHeader, classes.Min768TableCell].join(' ')}>
                            <label className={classes.TermNameSelectWrapper}>
                                Term
                                <select className={classes.TermNameSelect} value={termName} onChange={(event) => { setTermName(event.target.value); }}>
                                    {TERM_NAMES.map(termName => <option key={`${termName}1`} value={termName}>{termName}</option>)}
                                </select>
                            </label>
                        </th>
                    ...
                    {fetchLoading || fetchError ? null : courses.map((course, index) => (
                        <Course 
                            key={course.name} 
                            style={index % 2 === 1 ? {backgroundColor: '#e6e6e6'} : null}
                            term={course.term}
                            name={course.name}
